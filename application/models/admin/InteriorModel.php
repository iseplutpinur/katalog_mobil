...
class InteriorModel extends CI_Model
{
  private $foto_path = './files/interior/';
  private $cache_name = 'interior';
  ...
  {
    $return = $this->db->select('foto')->from('ktm_interior')->where('id', $id)->get()->row_array();
    ...
    if ($return != null) {
      ...
      // save database
      $this->db->where('id', $id);
      $res_data = $this->db->update('ktm_interior', $data);
  ...
  public function delete($id)
  {
    ...
    if ($return != null) {
      ...
      // from database foto
      $res_data = $this->db->delete('ktm_interior', ['id' => $id]);
