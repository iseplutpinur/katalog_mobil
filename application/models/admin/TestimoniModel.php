...
{
  private $foto_path = './files/testimoni/';
  private $cache_name = 'testimoni';
  ...
  public function update(
    ...
    $return = $this->db->select('foto')->from('ktm_testimoni')->where('id', $id)->get()->row_array();
    if ($return != null) {
      $data = [
        'title' => $title,
        'status' => $status,
      ...
      // save database
      $this->db->where('id', $id);
      $res_data = $this->db->update('ktm_testimoni', $data);
  ...
  public function delete($id)
  {
    ...
    if ($return != null) {
      ...
      // from database foto
      if ($return['foto'] != '' && $return['foto'] != null) {
        $res_data = $this->db->delete('ktm_testimoni', ['id' => $id]);
      }
      $this->cache->delete($this->cache_name);
      $this->cache->delete($this->cache_name . "_" . $id);
  ...
  private function saveFile()
  {
    ...
    $config['file_name']            = md5(uniqid("ktm_testimoni", true));
